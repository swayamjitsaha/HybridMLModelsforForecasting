...
# Reshape input into 3D form [samples, timesteps, features]
...
'''
lstm_input = Input(shape=(train_X.shape[1], train_X.shape[2]))  # setting the input shape 
lstm_output = LSTM(10, return_sequences=True)(lstm_input)       # 1st layer of the network
lstm_output = LSTM(10)(lstm_output)                             # 2nd layer of the network
dense_output = Dense(n_out)(lstm_output)                        # output layer of the network
...
history = model.fit(train_X, train_y, epochs=100, validation_data=(test_X, test_y), 
               callbacks=callbacks_list, batch_size=52, verbose=2, shuffle=False)
...
plt.title('Hour Vs GHI for Orissa(October) - StackedStatelessGRU')
plt.xlabel('Hour')
